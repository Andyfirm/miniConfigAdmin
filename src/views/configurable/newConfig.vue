    ...
    <el-card class="tableInfo">
        ...
        >
          <el-form-item label="数据源" prop="clubMemberCode">
            <el-select
              ...
              placeholder="请选择"
              @change="changeclubMemberCode(formparams.clubMemberCode)"
            >
              <el-option
                v-for="item in this.dataTypeList"
                :key="item.clubMemberCode"
          ...
          <el-form-item label="推送类型" prop="type">
            <el-select
              ...
              placeholder="请选择"
              @change="getTypeShortName(formparams.type)"
            >
              <el-option
                v-for="item in this.pushTypeS"
                :key="item.typeShortName"
                :label="item.type"
                :value="item.typeShortName"
              ></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-row class="tableList">
        ...
        >
          ...
          <el-table-column prop="remark" label="推送类型"></el-table-column>
...
import {
  getDataSource,
  getTbPushMessageModel,
  getPushType,
  insertTbPushMessageModel,
  deleteTbPushMessageModel,
  updateTbPushMessageModel,
} from "../../api/index";
...
export default {
  ...
  methods: {
    ...
    getTypeShortName(val) {
      for (var i = 0; i < this.pushTypeS.length; i++) {
        if (this.pushTypeS[i].typeShortName == val) {
          this.formparams.remark = this.pushTypeS[i].type;
        }
      }
    },
    //查询推送类型
    getPushType() {
      getPushType({ clubMemberCode: this.formparams.clubMemberCode }).then(
        (res) => {
          this.pushTypeS = res.data;
        }
      );
    },
    getTableList() {
      getTbPushMessageModel({ clubMemberCode: this.clubMemberCode }).then(
        (res) => {
          this.loading = false;
    ...
    edit(row) {
      this.addType = 2;
      this.formparams.type = row.remark;
      this.formparams.modelid = row.modelid;
      this.formparams.remark = row.remark;
      this.dialogVisible = true;
    ...
    submitForm(formparams) {
      this.$refs[formparams].validate((valid) => {
        if (valid) {
          if (this.addType == 1) {
            insertTbPushMessageModel(this.formparams).then((res) => {
              if (res.msg == "success") {
                this.$message({
                  type: "success",
                  message: "添加成功",
            ...
            for (let i = 0; i < this.pushTypeS.length; i++) {
              if (this.formparams.remark == this.pushTypeS[i].type) {
                this.formparams.type = this.pushTypeS[i].typeShortName;
                // this.formparams.remark =  this.pushTypeS[i].type
              }
            }
            updateTbPushMessageModel(this.formparams).then((res) => {
              if (res.msg == "success") {
                this.$message({
                  type: "success",
                  message: "修改成功",
